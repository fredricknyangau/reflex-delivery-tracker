...
class DeliveryService:
    ...
    async def create_request(
        ...
        async with DatabaseConnection.transaction() as conn:
            retailer = await conn.fetchrow(
                "SELECT id FROM retailers WHERE id = $1", retailer_id
            )
            if not retailer:
                raise KeyError(f"Retailer with id {retailer_id} not found.")

            user = await conn.fetchrow(
                "SELECT id FROM users WHERE id = $1", created_by
            )
            if not user:
            ...
            """
            await conn.execute(event_sql, request_dict["id"], created_by)

            return request_dict

    @classmethod
    async def assign_rider(
        cls, request_id: int, assigned_rider_id: int, changed_by: int
    ) -> dict[str, Any]:
        async with DatabaseConnection.transaction() as conn:
            rider = await conn.fetchrow(
                "SELECT id, role FROM users WHERE id = $1", assigned_rider_id
            )
            ...
            """
            row = await conn.fetchrow(
                update_sql, assigned_rider_id, request_id
            )

            if not row:
                existing = await conn.fetchrow(
                    "SELECT id, status, assigned_rider_id FROM delivery_requests WHERE id = $1",
                    request_id,
                )
                if not existing:
                    raise KeyError(f"Delivery request {request_id} not found.")
                raise ValueError(
                    f"Delivery request {request_id} is already in status '{existing['status']}' and cannot be assigned."
                )
            ...
            event_sql = """
                ...
                VALUES ($1, 'Assigned', $2)
            """
            await conn.execute(event_sql, request_id, changed_by)
    ...
    async def mark_picked_up(cls, request_id: int, changed_by: int) -> dict[str, Any]:
        async with DatabaseConnection.transaction() as conn:
            existing = await conn.fetchrow(
                "SELECT * FROM delivery_requests WHERE id = $1", request_id
            )
            if not existing:
                raise KeyError(f"Delivery request {request_id} not found.")
            ...
            if existing["status"] != "Assigned":
                raise ValueError(
                    f"Cannot pick up delivery request in status '{existing['status']}'. Must be Assigned."
                )

            if existing["assigned_rider_id"] != changed_by:
                raise PermissionError(
                    f"User {changed_by} is not the assigned rider for request {request_id}."
                )
            ...
            """
            row = await conn.fetchrow(
                update_sql, code, request_id, changed_by
            )
            if not row:
    ...
    async def mark_delivered(
        cls, request_id: int, changed_by: int, confirmation_code: str | None
    ) -> dict[str, Any]:
        async with DatabaseConnection.transaction() as conn:
            existing = await conn.fetchrow(
                "SELECT * FROM delivery_requests WHERE id = $1", request_id
            ...
            if existing["status"] != "Picked Up":
                raise ValueError(
                    f"Cannot deliver request in status '{existing['status']}'. Must be Picked Up."
                )
            ...
            if (
                not confirmation_code
                or confirmation_code.strip() != existing["confirmation_code"]
            ):
            ...
            """
            row = await conn.fetchrow(
                update_sql, request_id, changed_by, confirmation_code.strip()
            )
            if not row:
    ...
    @classmethod
    async def get_request(cls, request_id: int) -> dict[str, Any] | None:
    ...
    @classmethod
    async def get_history(cls, request_id: int) -> list[dict[str, Any]] | None:
        existing = await DatabaseConnection.fetchrow(
            "SELECT id FROM delivery_requests WHERE id = $1", request_id
        )
        if not existing:
            return None

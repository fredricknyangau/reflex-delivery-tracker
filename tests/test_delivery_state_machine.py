...
class MockDatabaseConnection:
    retailers = [{'id': 1, 'business_name': 'Test Retailer', 'phone': '0700000000', 'address': 'Nairobi'}]
    users = [
        {'id': 1, 'retailer_id': 1, 'name': 'Alice Retailer', 'phone': '0711111111', 'role': 'retailer_staff'},
        {'id': 2, 'retailer_id': 1, 'name': 'Bob Dispatcher', 'phone': '0722222222', 'role': 'dispatcher'},
        {'id': 3, 'retailer_id': 1, 'name': 'Brian Rider', 'phone': '0733333333', 'role': 'rider'},
        {'id': 4, 'retailer_id': 1, 'name': 'Other Rider', 'phone': '0744444444', 'role': 'rider'},
    ]
    delivery_requests = {}
    status_events = []
    next_request_id = 1
    ...
    async def fetchrow(cls, query: str, *args):
        ...
        if 'UPDATE DELIVERY_REQUESTS' in q:
            if 'DELIVERED' in q and 'STATUS' in q and 'SET' in q:
                ...
                if req and req['status'] == 'Picked Up' and req['assigned_rider_id'] == rider_id and req['confirmation_code'] == code:
            ...
            if 'PICKED UP' in q and 'STATUS' in q and 'SET' in q:
                ...
                req = cls.delivery_requests.get(req_id)
                if req and req['status'] == 'Assigned' and req['assigned_rider_id'] == rider_id:
        ...
        if 'FROM STATUS_EVENTS' in q:
            ...
            for ev in cls.status_events:
                if ev['delivery_request_id'] == req_id:
                    u = next((u for u in cls.users if u['id'] == ev['changed_by']), {'name': 'Unknown', 'role': 'unknown'})
                    res.append({
                        'id': ev['id'],
...
async def test_suite():
    ...
    req = await DeliveryService.create_request(1, 1, 'Margaret Nduta', '0700112233', 'Ngong Road', 'Cooking Oil')
